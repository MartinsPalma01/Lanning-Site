(function () {
  ...
  function apiPath(path) {
    const routes = {
      ...
      "/api/admin/leads": "/api/admin-leads",
      "/api/admin/users": "/api/admin-users",
    };
    const leadMatch = String(path).match(/^\/api\/admin\/leads\/([^/]+)$/);
    if (leadMatch) return `/api/admin-leads?id=${encodeURIComponent(leadMatch[1])}`;
    return routes[path] || path;
  }

  function leadDocumentPath(leadId, index) {
    return `/api/admin-lead-document?id=${encodeURIComponent(leadId)}&index=${encodeURIComponent(index)}`;
  }
  ...
  function renderContacts() {
    ...
    box.innerHTML = `<table><thead><tr><th>Data</th><th>Nome</th><th>Contato</th><th>Área</th><th>Urgência</th><th>Profissional</th><th>Status</th><th>Documentos</th><th>Ações</th></tr></thead><tbody>${leads.map((row) => {
      ...
      const docs = (row.documentos || []).map((doc) => `<a href="${escapeHtml(leadDocumentPath(row.id, doc.index))}" target="_blank" rel="noopener">${escapeHtml(doc.name)}</a>`).join("<br>");
